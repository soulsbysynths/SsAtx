...
class stk500
{
	...
	public:
	static void disable();
	static int program_enable();
	static int getsync();
	static int getparm(unsigned parm, unsigned * value);
	static int drain();
	static int loadaddr(unsigned int addr);
	static int paged_write(AVRMEM * m, int page_size, int n_bytes);
	stk500();
	~stk500();
	protected:
	private:
	static int send(byte *buf, unsigned int len);
	static int recv(byte * buf, unsigned int len);
	...
	static int paged_load(AVRMEM * m, int page_size, int n_bytes);
	#endif
	static void error1(int errno,unsigned char detail);
	static void error2(int errno);
	static void dumphex(unsigned char *buf,int len);
	stk500( const stk500 &c );
	stk500& operator=( const stk500 &c );
